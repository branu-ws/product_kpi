"""経営管理 Fan / Proactive 顧客の機能別週次利用数を個社別に可視化する。

対象  : keiei_monthly_company で一度でも fan/proactive の会社 (sf_customers = Plus のみ)
facet : 機能 (案件ステータス更新 / OCR処理 / 実績原価登録 / 実績売上登録 /
              見積売上登録 / 原価ページPV / 見積原価登録 / 請求書発行)
color : 会社名
UI    : 検索付きリストボックスで 1 社 or 全社を切り替え

出力  : output/html/keiei_company_feature_weekly.html
"""
...
import kpi.db as db
from kpi.config import KEIEI_FEATURE_THRESHOLDS, TIER
...
all_companies AS (
    SELECT DISTINCT h.company_uuid, c.company_name
    FROM keiei_user_history h
    JOIN ever_fan_proactive epf USING (company_uuid)
...
def to_score(count, feature):
    if pd.isna(count):
        return None  # 線を切る
    t = KEIEI_FEATURE_THRESHOLDS.get(feature)
    if t is None:
        return 0
...
out_path = out_dir / "keiei_company_feature_weekly.html"
