"""X-PRODUCT 顧客 Tier の月次推移を Plotly で可視化する。

出力: output/html/xproduct_tier_trend.html
"""
...
fig.update_layout(
    title=dict(
        text="X-PRODUCT 顧客 Tier 推移（月次・社数）",
        font=dict(size=18),
    ),
    xaxis=dict(
        title="月",
    ...
    yaxis=dict(title="社数", showgrid=True, gridcolor="#eeeeee"),
...
out_path = out_dir / "xproduct_tier_trend.html"
