"""Work (施工管理) 顧客 Tier × Health Score の週次推移を Plotly Express facet で可視化する。

facet_row=diversity_tier, color=usage_freq
サブプロット背景: Tier カラーの暗めバージョン
Health Score: 黒系グラデーション (good=濃 / bad=薄)

出力: output/html/work_tier_health_trend_weekly.html
"""
...
WITH actual AS (
    ...
    FROM work_company_weekly
    GROUP BY 1, 2, 3
),
scaffold AS (
    SELECT w.week_start, t.diversity_tier, f.usage_freq
    FROM (SELECT DISTINCT week_start::DATE AS week_start FROM work_company_weekly) w
    CROSS JOIN (SELECT DISTINCT diversity_tier FROM work_company_weekly) t
    CROSS JOIN (SELECT DISTINCT usage_freq     FROM work_company_weekly) f
)
...
fig = px.area(
    ...
    title="Work (施工管理) 顧客 Tier × Health Score 週次推移",
    height=900,
    width=1200,
...
out_path = out_dir / "work_tier_health_trend_weekly.html"
