"""出面・写真アップロード・フォルダ作成・AIアシスタントのトレンド
facet_row=Plus/Mini, facet_col=顧客数/作成数, color=機能
"""
...
-- 出面: work_user_history → pid 経由で company_uuid に変換
attendance AS (
    SELECT
...
-- 写真・フォルダ: contents_user_history は company_uuid 直接
contents AS (
    SELECT
        strftime(content_date, '%Y-%m') AS month,
        company_uuid,
...
-- AI: ai_user_history は company_uuid 直接
ai AS (
    SELECT
        strftime(content_date, '%Y-%m') AS month,
        company_uuid,
...
all_events AS (
    SELECT month, company_uuid, feature FROM attendance
    UNION ALL
    SELECT month, company_uuid, feature FROM contents
    UNION ALL
...
SEGMENTS = ["Plus", "Mini"]
FEATURES = ["出面", "写真アップロード", "フォルダ作成", "AIアシスタント"]
COLORS = {
    "出面": "#1f77b4",
    "写真アップロード": "#2ca02c",
    "フォルダ作成": "#ff7f0e",
...
fig.update_layout(
    title="機能別利用トレンド (出面・写真・フォルダ・AI) — Plus / Mini × 顧客数 / 作成数",
    height=650,
    width=1050,
...
out = "output/work_user_trend/work_user_trend.html"
