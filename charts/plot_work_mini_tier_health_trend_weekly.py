"""Work Mini 顧客 Tier × Health Score の週次推移を Plotly Express facet で可視化する。

facet_row=diversity_tier, color=usage_freq
サブプロット背景: Tier カラーの暗めバージョン
Health Score: 黒系グラデーション (good=濃 / bad=薄)

出力: output/html/work_mini_tier_health_trend_weekly.html
"""
...
df = conn.sql("""
WITH mini_only AS (
    -- mini 契約あり かつ Plus 契約なし (Plus に寄せるため sf_customers から除外)
...
scaffold AS (
    ...
    FROM (SELECT DISTINCT week_start::DATE AS week_start FROM mini_work_company_weekly) w
...
fig = px.area(
    ...
    title="Work Mini 顧客 Tier × Health Score 週次推移",
    height=900,
...
out_path = out_dir / "work_mini_tier_health_trend_weekly.html"
